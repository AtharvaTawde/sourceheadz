...
public class Trader : MonoBehaviour {
    ...
    private string[] traderTypes = {"Forger", "Rancher"};
    private string hoveringOver;
    private int currentIndexShowing = 0; 
    private string traderType;

    [SerializeField] Sprite[] forgerItems;   
    [SerializeField] Sprite[] rancherItems;   
    [SerializeField] Sprite[] brewerItems;   
    ...
    public void Trade() {
        if (tradingSlot.Item != null && tradingSlot.Item.ItemName == reactant && tradingSlot.Amount >= reactantAmount) {
            if (Input.GetKey(KeyCode.RightShift) || Input.GetKey(KeyCode.LeftShift)) {
                for (int i = 0; i < tradingSlot.Amount; i++) {
                    TradeItem();
                } 
            } else {
                TradeItem();
    ...
    private void Update() {
        ...
        string pureP = traderType == "Forger" ? allForgerTradesPossible[currentItemsSelling[currentIndexShowing]] : allRancherTradesPossible[currentItemsSelling[currentIndexShowing]];
        ...
        hoverOverSelf = hit.collider == GetComponent<Collider2D>() && hit.collider != null ? true : false;

        if (hoverOverSelf && Input.GetMouseButtonDown(1) && !isTradingMenuActive && !c.isCraftingMenuActive) {
            OpenTradingMenu();
            c.ToggleCraftingMenu();
            isTradingMenuActive = true;
        } else if (((Input.GetMouseButtonDown(1) && !HelperFunctions.IsPointerOverUIElement()) || Input.GetKeyDown(KeyCode.C)) && isTradingMenuActive) {
            CloseTradingMenu();
            c.ToggleCraftingMenu();
