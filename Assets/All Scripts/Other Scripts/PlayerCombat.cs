...
public class PlayerCombat : MonoBehaviour {
    ...
    public float voracity;
    public float hypermobility;
    public float autoimmunity;
    #endregion
    ...
    private float floatyScale = -0.1f;
    private float resistanceConstant = 1f;
    private float voracityConstant = 1f;
    ...
    void Update() {
        agility = agility > 0 ? agility -= Time.deltaTime : 0;
        longevity = longevity > 0 ? longevity -= Time.deltaTime : 0;
        voracity = voracity > 0 ? voracity -= Time.deltaTime : 0;
        hypermobility = hypermobility > 0 ? hypermobility -= Time.deltaTime : 0;
        autoimmunity = autoimmunity > 0 ? autoimmunity -= Time.deltaTime : 0;
        ...
        if (!GetComponent<PlayerMovement>().isPaused && !GetComponent<PlayerMovement>().isDead) {
            ...
            resistanceConstant = longevity > 0 ? 1.75f : 1f; 
            voracityConstant = voracity > 0 ? 1.75f : 1f; 
            ...
            #region Auto Regeneration and Auto Immunity Effects
            if (autoimmunity <= 0) {                            // without autoimmunity potion on
            ...
            } else {                                            // with autoimmunity potion on
                if (saturationHealTime > 0) {
                    saturationHealTime -= Time.deltaTime;
                } else {
                    saturationHealTime = 0;
                    currentHealth += 5;
                    saturationHealTime = 0.625f;
                }
            } 
            #endregion
    ...
    public void TakeDamage(int damage) {
        ...
        hurt = true;
        currentHealth -= Mathf.RoundToInt(damage * (1 - damageReduction) * (1f / resistanceConstant));
    ...
    public void PlayerDie() {
        ...
        deaths += 1;
        agility = 0; longevity = 0; voracity = 0; hypermobility = 0; autoimmunity = 0;
    }
    ...
    public void Attack() {
        ...
        foreach (Collider2D[] set in colliders) {
            foreach(Collider2D c in set) {
                ...
                int onlyPEAO = enemies.value | plants.value | animals.value | obstacles.value;
                RaycastHit2D hit = Physics2D.Linecast(attackPoint.position, c.transform.position, onlyPEAO);
                if (hit.collider.gameObject == c.gameObject) {                                                  // if staring at the target directly (no obstructions), deal damage
                    c.GetComponent<TakeDamage>().ReceiveDamage(Mathf.RoundToInt(atkdmg * voracityConstant));
                } else {
                    c.GetComponent<TakeDamage>().ReceiveDamage(Mathf.RoundToInt(1 * voracityConstant));
                }

                if (c.GetComponent<Rigidbody2D>() != null) {                                                // if has rb2d, do knockback away from player

                    Vector3 hitDirection = transform.position - c.transform.position;
                    Rigidbody2D crb = c.GetComponent<Rigidbody2D>();
                    float _mass = crb.mass;

                    crb.AddForce(-hitDirection * 175 * _mass);  
                    crb.AddForce(Vector2.up * 175 * _mass);                  
        ...
        foreach(Collider2D collider in hitBosses) {
            collider.GetComponent<TakeDamage>().ReceiveDamage(Mathf.RoundToInt(atkdmg * voracityConstant));
        }
        ...
        // Item Pickup
        string[] itemNames = {"Anthocyanin", "Apple", "Baton", "Blood Pie", "Blue Crystal", "Boss Gem", "Bread Scrap", "Chicken", "Clay Idol", 
                              "Clay Slab", "Clownfish", "Coal Dust", "Cooked Chicken", "Cooked Clownfish", "Cooked Goblin Meat", "Cooked Nyert Meat", 
                              "Cooked Zombie Spleen", "Corpuscle", "Crimson Leaf", "Diamond", "Diamond Boots", "Diamond Chestplate", 
                              "Diamond Leggings", "Diamond Sword", "Egg", "Empty Bottle", "Eyeball", "Fabric", "Fireball", "Glass Shard", 
                              "Goblin Meat", "Gold Bar", "Gold Scrap", "Green Crystal", "Gunflower Node", "Gunflower Seed", "Haunted Leaf",
                              "Health Pot", "Leaf", "Jump Potion", "Nyert Meat", "Oven", "Phlegm Lattice", "Popper", "Red Crystal", "Speed Potion", 
                              ...
                              "Potion of Agility", "Potion of Autoimmunity", "Potion of Fire Resistance", "Potion of Hypermobility", "Potion of Longevity", "Potion of Voracity",
                              "Potion of Agility²", "Potion of Autoimmunity²", "Potion of Fire Resistance²", "Potion of Hypermobility²", "Potion of Longevity²", "Potion of Voracity²",
                              "Potion of Agility³", "Potion of Autoimmunity³", "Potion of Fire Resistance³", "Potion of Hypermobility³", "Potion of Longevity³", "Potion of Voracity³"};
    ...
    private void OnTriggerStay2D(Collider2D other) {
        ...
        if (other.gameObject.tag == "Gateway") {
            if (other.transform.localScale == Vector3.one * 2) {
                transform.position = new Vector3(962f, 1451.5f, 0f);
            }
        }
    }
