...
public class Traveler : MonoBehaviour  {
    ...
    void Start() {
        ...
        tradeCooldown = Random.Range(3, 5);
    }
    ...
    void Update() {
        ...
        // Barter if Hand is Full
        if (heldItem.GetComponent<SpriteRenderer>().sprite != null) {
            if (tradeCooldown > 0) {
    ...
    void SetupBarter(Collider2D pickup) {
    ...
    void Barter() {
        ...
        if (transform.eulerAngles == Vector3.zero) {
            item.GetComponent<Rigidbody2D>().AddForce(Vector2.left * 350f + Vector2.up * 100f);
        } else {
            item.GetComponent<Rigidbody2D>().AddForce(Vector2.right * 350f + Vector2.up * 100f);
        }
